...
import { searchHealthcareEstablishments } from "../api/overpassApi";
...
export default function MapWithMarkers({
    ...
    onLoadingStateUpdate, // Callback pour synchroniser l'état de chargement
}) {
    ...
    }, [userPosition, searchQuery, isLoading]);
    ...
    const handleManualSearch = useCallback(() => {
        ...
        searchHealthcareEstablishments(userPosition, 5000, trimmedQuery)
            .then((establishments) => {
                setRealTimeItems(establishments);
                if (onItemsUpdateRef.current) {
                    onItemsUpdateRef.current(establishments);
                }
            })
            .catch(() => {
                setRealTimeItems([]);
    ...
    }, [canSearch, searchQuery, lastSearchQuery, userPosition]);
